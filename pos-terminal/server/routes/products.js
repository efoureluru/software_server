...
router.put('/:id', async (req, res) => {
    try {
        // Try finding by custom 'id' first, fallback to _id if valid
...
router.delete('/:id', async (req, res) => {
    try {
        // Try deleting by custom 'id' first, fallback to _id if valid
        let success = await Product.findOneAndDelete({ id: req.params.id });

        if (!success && mongoose.Types.ObjectId.isValid(req.params.id)) {
            success = await Product.findByIdAndDelete(req.params.id);
        }
