            ...
            <header className="bg-slate-900 text-white p-4 shadow-xl z-10 border-b border-white/10">
                <div className="container mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        {/* Only show Back button if NOT a dedicated scanner user */}
                        {JSON.parse(localStorage.getItem('user') || '{}').role !== 'scanner' && (
                            <button
                    ...
                    {JSON.parse(localStorage.getItem('user') || '{}').role === 'scanner' && (
                        <button
                            ...
                            className="bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 hover:text-rose-400 px-4 py-2 rounded-xl font-bold text-sm transition-all border border-rose-500/20"
                        >
                            Logout
                        </button>
                    )}
